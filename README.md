...
[EcoTyper](https://ecotyper.stanford.edu/) is a machine learning framework for large-scale identification of cell type-specific transcriptional states and their co-association patterns from bulk, single-cell, and spatially resolved gene expression data. EcoTyper performs the following major steps:
...
-   Cell state and ecotype recovery: This step enables recovery of cell states and ecotypes in external expression datasets.

We have already defined cell states and ecotypes both across **carcinomas** and in **diffuse large B cell lymphoma (DLBCL)**. The current version of EcoTyper allows users to recover the cell states and ecotypes for these two tumor categories in their own data. Additionally, it allows users to discover and recover cell states and ecotypes in their system of interest. Below we illustrate each of these functionalities.

## Download

The latest version of EcoTyper source code can be found on [EcoTyper GitHub repository](https://github.com/digitalcytometry/ecotyper) and [Ecotyper website](https://ecotyper.stanford.edu/).

## Setup

The software below is required for running EcoTyper. The version numbers indicate the package versions used for developing and testing the EcoTyper code. Other R versions might work too:
...
-   Docker, if *de novo* discovery of cell states and ecotypes is planned (Tutorial 4), or if cell state and ecotype recovery in spatial transcriptomics is performed (Tutorial 3).

To set up EcoTyper, please download this folder locally. The resources pre-stored in the EcoTyper folder allow users to perform the recovery of cell states and ecotypes previously defined in carcinoma or lymphoma, in their own bulk RNA-seq, microarray and scRNA-seq data, using R only. If, however, users plan to perform *de novo* identification of cell states and ecotypes in their own system of interest, or perform cell state and ecotype recovery in spatial transcriptomics assays, CIBERSORTx executables need to be downloaded from the [CIBERSORTx website](https://cibersortx.stanford.edu/). Specifically, EcoTyper requires the **CIBERSORTx Fractions** and **CIBERSORTx HiRes** modules. The executables for these two modules are available for download as Docker images on the [CIBERSORTx website](https://cibersortx.stanford.edu/). Please follow the instructions on the [Download](https://cibersortx.stanford.edu/download.php) section of the website to download the Docker images and obtain the Docker tokens necessary for running them.

## EcoTyper implementation

EcoTyper is implemented in R. Some of the EcoTyper functions are computationally intensive, especially for the cell state discovery step described in Tutorial 4. Therefore, EcoTyper is designed as a collection of modular command-line R scripts, that can be run in parallel on a multi-processor server or a high-performance cluster. Each script is designed such that its instances can typically be run on a single core.

We provide wrappers over these scripts, that encapsulate the typical EcoTyper workflows (*Tutorials 1-4*). These wrappers can be run on a multi-core system, and allow users to discover cell states and ecotypes in their own data, as well as recover previously discovered cell states and ecotypes in bulk tissue expression profiles, spatial transcriptomics assays, and single-cell RNA-seq data.

We provide 4 tutorials illustrating EcoTyper usage:

-   **Tutorial 1:** Recovery of Cell States and Ecotypes Pre-Defined for Carcinoma in User-Provided Data
-   **Tutorial 2:** Recovery of Cell States and Ecotypes Pre-Defined for Lymphoma in User-Provided Data
-   **Tutorial 3:** Cell state and ecotype recovery in Visium Spatial Gene Expression data
-   **Tutorial 4:** *De novo* Discovery of Cell States and Ecotypes

## **Tutorial 1:** Recovery of Cell States and Ecotypes Pre-Defined for **Carcinoma** in User-Provided Data

EcoTyper comes pre-loaded with the resources necessary for the reference-guided recovery of cell states and ecotypes previously defined in carcinoma or lymphoma, in user-provided expression data.

In this tutorial, we illustrate how EcoTyper can be used to recover the cell states and ecotypes that we defined across **carcinomas**, in a downsampled version of a [scRNA-seq dataset](https://www.nature.com/articles/s41591-019-0750-6) from lung cancer specimens, and a subset of the TCGA bulk samples from lung adenocarcinoma (LUAD).

### 1.1. Recovery of **Carcinoma** Cell States in scRNA-seq Data

In this section we illustrate how carcinoma cell states can be recovered in a scRNA-seq dataset from lung cancer specimens. The expression data used in this tutorial can be found in `example_data/carcinoma_recovery_scRNA_data.txt`, and its corresponding sample annotation in `example_data/carcinoma_recovery_scRNA_annotation.txt`.
...
``` r
data = read.delim("example_data/carcinoma_recovery_scRNA_data.txt", nrow = 5)
head(data[,1:5])
```
...
``` bash
Rscript EcoTyper_recovery_scRNA.R -d Carcinoma -m example_data/carcinoma_recovery_scRNA_data.txt -a example_data/carcinoma_recovery_scRNA_annotation.txt -o RecoveryOutput
```
...
``` r
data = read.delim("RecoveryOutput/carcinoma_recovery_scRNA_data/Epithelial.cells/state_assignment.txt")
...
``` r
knitr::include_graphics("RecoveryOutput/carcinoma_recovery_scRNA_data/Epithelial.cells/state_assignment_heatmap.png")
```

<img src="RecoveryOutput/carcinoma_recovery_scRNA_data/Epithelial.cells/state_assignment_heatmap.png" width="100%" style="display: block; margin: auto;" />
...
``` r
data = read.delim("RecoveryOutput/carcinoma_recovery_scRNA_data/Epithelial.cells/recovery_z_scores.txt")
head(data[,c("State", "Z")])
```
...
### 1.2. Recovery of **Carcinoma** Cell States and EcoTypes in Bulk Data

In this section, we illustrate how EcoTyper can be used to recover cell states end ecotypes in a new bulk RNA-seq or microarray dataset. In the carcinoma EcoTyper paper, we demonstrate that prior deconvolution of bulk data using *CIBERSORTx HiRes* is not necessary for high-fidelity recovery of cell states in bulk-tissue expression data. We can proceed to the recovery of states based on bulk data only.

For this tutorial, we used a subset of the TCGA bulk samples from lung adenocarcinoma (LUAD), available in `example_data/carcinoma_recovery_bulk_data.txt`, together with the sample annotation file `example_data/carcinoma_recovery_bulk_annotation.txt`.
...
``` r
data = read.delim("example_data/carcinoma_recovery_bulk_data.txt", nrow = 5)
head(data[,1:5])
```

    ##      Gene TCGA.44.3398.01A.01R.1107.07 TCGA.78.8640.01A.11R.2403.07
    ## 1    A1BG                 2.187385e+01                  20.25238683
    ## 2    A1CF                 6.561076e-03                   0.05419415
    ## 3     A2M                 3.338875e+02                 112.24231014
    ## 4   A2ML1                 1.364466e-01                   5.52654105
    ## 5 A3GALT2                 1.866910e-02                   0.00000000
    ##   TCGA.49.6744.11A.01R.1858.07 TCGA.97.A4M2.01A.12R.A24X.07
    ## 1                 6.696458e+00                  32.75981082
    ## 2                 0.000000e+00                   0.02489151
    ## 3                 1.593506e+03                 648.09240219
    ## 4                 3.457812e-01                   0.94368062
    ## 5                 4.674692e-02                   0.00000000
...
``` r
data = read.delim("example_data/carcinoma_recovery_bulk_annotation.txt")
head(data)
```

    ##                             ID Histology Tissue OS_Time OS_Status
    ## 1 TCGA.44.3398.01A.01R.1107.07      LUAD  Tumor    1163         0
    ## 2 TCGA.78.8640.01A.11R.2403.07      LUAD  Tumor    7062         0
    ## 3 TCGA.49.6744.11A.01R.1858.07      LUAD Normal    1683         0
    ## 4 TCGA.97.A4M2.01A.12R.A24X.07      LUAD  Tumor     624         0
    ## 5 TCGA.49.4512.11A.01R.1858.07      LUAD Normal     905         1
    ## 6 TCGA.75.6205.01A.11R.1755.07      LUAD  Tumor      NA         1
...
``` bash
Rscript EcoTyper_recovery_bulk.R -d Carcinoma -m example_data/carcinoma_recovery_bulk_data.txt -a example_data/carcinoma_recovery_bulk_annotation.txt -c Tissue -o RecoveryOutput
```
...
``` r
data = read.delim("RecoveryOutput/carcinoma_recovery_bulk_data/Fibroblasts/state_abundances.txt")
head(data[,1:5])
```
...
``` r
data = read.delim("RecoveryOutput/carcinoma_recovery_bulk_data/Fibroblasts/state_assignment.txt")
head(data[,c("ID", "State")])
```
...
``` r
knitr::include_graphics("RecoveryOutput/carcinoma_recovery_bulk_data/Fibroblasts/state_assignment_heatmap.png")
```

<img src="RecoveryOutput/carcinoma_recovery_bulk_data/Fibroblasts/state_assignment_heatmap.png" width="100%" style="display: block; margin: auto;" />
...
``` r
assign = read.delim("RecoveryOutput/carcinoma_recovery_bulk_data/Ecotypes/ecotype_abundance.txt")
dim(assign)
```
...
```
    ...
    ##     TCGA.44.3398.01A.01R.1107.07 TCGA.78.8640.01A.11R.2403.07
    ## CE1                   0.28440820                  0.014656332
    ## CE2                   0.11109890                  0.152232903
    ## CE3                   0.19505752                  0.016229465
    ## CE4                   0.05326474                  0.097930867
    ## CE5                   0.03882698                  0.197868279
    ## CE6                   0.12670608                  0.009292463
    ##     TCGA.49.6744.11A.01R.1858.07 TCGA.97.A4M2.01A.12R.A24X.07
    ## CE1                   0.04545839                   0.01167567
    ## CE2                   0.01427041                   0.02825505
    ## CE3                   0.11002993                   0.10726523
    ## CE4                   0.06296961                   0.12072879
    ## CE5                   0.01423865                   0.08029813
    ## CE6                   0.63435171                   0.19723199
    ##     TCGA.49.4512.11A.01R.1858.07
    ## CE1                   0.02127565
    ## CE2                   0.01236350
    ## CE3                   0.15762297
...
``` r
discrete_assignments = read.delim("RecoveryOutput/carcinoma_recovery_bulk_data/Ecotypes/ecotype_abundance.txt")
...
``` r
knitr::include_graphics("RecoveryOutput/carcinoma_recovery_bulk_data/Ecotypes/heatmap_assigned_samples_viridis.png")
```

<img src="RecoveryOutput/carcinoma_recovery_bulk_data/Ecotypes/heatmap_assigned_samples_viridis.png" width="100%" style="display: block; margin: auto;" />
...
In this tutorial, we illustrate how EcoTyper can be used to recover the cell states and ecotypes, that we defined in **diffuse large B cell lymphoma** (DLBCL), in a downsampled version of a scRNA-seq dataset from lymphoma specimens, and a subset of the [bulk samples](https://www.nature.com/articles/s41591-018-0016-8) from diffuse large B cell lymphoma.

### 2.1. Recovery of **Lymphoma** Cell States in scRNA-seq Data

In this section we illustrate how lymphoma cell states can be recovered in the scRNA-seq dataset from lymphoma specimens. The expression data used in this tutorial can be found in `example_data/lymphoma_recovery_scRNA_data.txt`, and sample annotation in `example_data/lymphoma_recovery_scRNA_annotation.txt`.
...
-   *-d*/*--discovery*: The name of the discovery dataset used for defining cell states. By default, the only accepted values are *Carcinoma* and *Lymphoma* (case sensitive), which will recover the cell states that we defined in carcinoma and lymphoma, respectively. If the user defined cell states in their own data (**Tutorial 4**), the name of the discovery dataset is the value provided in the 'Discovery dataset name' filed of the configuration file used for running EcoTyper discovery ('EcoTyper\_discovery.R') script. In our tutorial, the name of the discovery dataset is *Lymphoma*.

-   *-m*/*--matrix*: Path to the input scRNA-seq matrix. The scRNA-seq expression matrix should be a tab-delimited file, with gene symbols on the first column and cells on the next columns. It should have cell identifiers (e.g. barcodes) as column names, and should be in TPM, CPM, FPKM or any other suitable count format. Gene symbols and cell identifiers should be unique. Moreover, we recommend that the column names do not contain special characters that are modified by the R function *make.names*, *e.g.* having digits at the beginning of the name or containing characters such as *space*, *tab* or *-*. The scRNA-seq data used in this tutorial looks as follows:

``` r
data = read.delim("example_data/lymphoma_recovery_scRNA_data.txt", nrow = 5)
head(data[,1:5])
```
...
``` bash
Rscript EcoTyper_recovery_scRNA.R -d Lymphoma -m example_data/lymphoma_recovery_scRNA_data.txt -a example_data/lymphoma_recovery_scRNA_annotation.txt -o RecoveryOutput -c Tissue
```
...
``` r
data = read.delim("RecoveryOutput/lymphoma_recovery_scRNA_data/B.cells/state_assignment.txt")
head(data[,c("ID", "State")])
```
...
``` r
knitr::include_graphics("RecoveryOutput/lymphoma_recovery_scRNA_data/B.cells/state_assignment_heatmap.png")
```

<img src="RecoveryOutput/lymphoma_recovery_scRNA_data/B.cells/state_assignment_heatmap.png" width="100%" style="display: block; margin: auto;" />

-   If the statistical significance quantification method is applied, the resulting z-score for each cell state are output in the same directory:

``` r
data = read.delim("RecoveryOutput/lymphoma_recovery_scRNA_data/B.cells/recovery_z_scores.txt")
head(data[,c("State", "Z")])
```
...
### 2.2. Recovery of **Lymphoma** Cell States and EcoTypes in Bulk Data

In this section, we illustrate how EcoTyper can be used to recover cell states end ecotypes in a new bulk RNA-seq or microarray dataset. In the carcinoma EcoTyper paper, we demonstrate that prior deconvolution of bulk data using **CIBERSORTx HiRes** is not necessary for high-fidelity recovery of cell states in user-provided bulk-tissue expression data. We can proceed to the recovery of states based on bulk data only.

For this tutorial, we used a subset of the [bulk samples](https://www.nature.com/articles/s41591-018-0016-8) from diffuse large-cell lymphoma (DLBCL), available in `example_data/lymphoma_recovery_bulk_data.txt`, together with the sample annotation file `example_data/lymphoma_recovery_bulk_annotation.txt`.
...
``` r
data = read.delim("example_data/lymphoma_recovery_bulk_data.txt", nrow = 5)
head(data[,1:5])
```
...
``` r
data = read.delim("example_data/lymphoma_recovery_bulk_annotation.txt")
head(data)
```
...
``` bash
Rscript EcoTyper_recovery_bulk.R -d Lymphoma -m example_data/lymphoma_recovery_bulk_data.txt -a example_data/lymphoma_recovery_bulk_annotation.txt -c schmitz_labels,COO -o RecoveryOutput
```
...
``` r
data = read.delim("RecoveryOutput/lymphoma_recovery_bulk_data/B.cells/state_abundances.txt")
head(data[,1:5])
```

    ##     MS2010072001 MS2010072003 MS2010072004 MS2010072017 MS2010072019
    ## S01 2.373248e-03 3.595619e-01 3.808712e-05 3.697861e-01 5.385938e-01
    ## S02 4.782425e-01 5.127358e-02 4.068437e-01 8.661441e-16 1.260097e-15
    ## S03 5.000272e-01 2.657651e-01 3.208191e-01 3.871465e-02 1.262400e-01
    ## S04 1.409287e-15 2.861734e-01 1.020793e-01 2.379055e-07 6.267285e-03
    ## S05 1.935706e-02 6.956557e-05 1.046191e-01 8.195355e-02 2.186160e-05

-   The assignment of samples to the state with highest abundance:

``` r
data = read.delim("RecoveryOutput/lymphoma_recovery_bulk_data/B.cells/state_assignment.txt")
head(data[,c("ID", "State")])
```
...
``` r
knitr::include_graphics("RecoveryOutput/lymphoma_recovery_bulk_data/B.cells/state_assignment_heatmap.png")
```

<img src="RecoveryOutput/lymphoma_recovery_bulk_data/B.cells/state_assignment_heatmap.png" width="100%" style="display: block; margin: auto;" />
...
``` r
assign = read.delim("RecoveryOutput/lymphoma_recovery_bulk_data/Ecotypes/ecotype_abundance.txt")
dim(assign)
```
...
```
    ...
    ##     MS2010072001 MS2010072003 MS2010072004 MS2010072017 MS2010072019
    ## LE1   0.04957053   0.08169862   0.08724605 7.549597e-02  0.039441745
    ## LE2   0.01006307   0.12841186   0.09457985 2.747771e-02  0.002941152
    ## LE3   0.05278895   0.03158041   0.07019043 2.622036e-01  0.246745155
    ## LE4   0.30161980   0.03494595   0.08382886 4.458163e-15  0.009809011
    ## LE5   0.17506190   0.19692139   0.14271844 1.226649e-01  0.137689548
    ## LE6   0.09394975   0.09523242   0.13293944 2.158502e-01  0.155547453

-   The assignment of samples to the lymphoma ecotype with the highest abundance. If the cell state fractions from the dominant ecotype are not significantly higher than the other cell state fractions in a given sample, the sample is considered unassigned and filtered out from this table:

``` r
discrete_assignments = read.delim("RecoveryOutput/lymphoma_recovery_bulk_data/Ecotypes/ecotype_abundance.txt")
dim(discrete_assignments)
```
...
``` r
knitr::include_graphics("RecoveryOutput/lymphoma_recovery_bulk_data/Ecotypes/heatmap_assigned_samples_viridis.png")
```

<img src="RecoveryOutput/lymphoma_recovery_bulk_data/Ecotypes/heatmap_assigned_samples_viridis.png" width="100%" style="display: block; margin: auto;" />

## **Tutorial 3:** Cell state and ecotype recovery in Spatial Transcriptomics data
...
This script takes as input file a configuration file in [YAML](https://yaml.org/) format. The configuration file for this tutorial is available in `visium_configuration.yml`:
...
``` r
data = read.delim("example_data/visium_cell_type_fractions_example.txt", nrow = 5)
dim(data)
```
...
``` yaml
Recovery cell type fractions : "example_data/visium_cell_type_fractions_example.txt"
```
...
After editing the configuration file (`visium_configuration.yml`), the command line for recovering the cell states and ecotypes in Visium Spatial Gene Expression data looks as illustrated below. Please note that this script might take up to two hours to run on 10 threads. Also, since CIBERSORTx is run on each spot, the memory requirements might exceed the memory available on a typical laptop. We recommend that this tutorial is run on a server with &gt;32GB of RAM.

``` r
Rscript EcoTyper_recovery_visium.R -c visium_configuration.yml
```
...
-   a user-provided bulk tissue expression matrix (RNA-seq or microarray), on which the discovery will be performed (a discovery cohort). For this tutorial, we will use the example data in `example_data/example_discovery_data.txt`.
...
-   optionally, a sample annotation file, such as the one provided in `example_data/example_discovery_annotation.txt`, can be supplied to EcoTyper. The information in this file can be used for heatmap plotting purposes, and also to instruct EcoTyper to find cell states/ecotypes common across different biological batches (e.g. tumor types), as detailed below.
...
``` bash
Rscript EcoTyper_discovery.R -h
```

    ## usage: EcoTyper_discovery.R [-c <PATH>] [-h]
    ## 
    ## Arguments:
    ##   -c <PATH>, --config <PATH>
    ##                         Path to the config files [required].
...
This script takes as input file a configuration file in [YAML](https://yaml.org/) format. The configuration file for this tutorial is available in `discovery.yml`:

``` yaml
default :
  Input :    
    Discovery dataset name : "MyDiscovery"
    Expression matrix : "example_data/example_discovery_data.txt"
    ...
    Annotation file : "example_data/example_discovery_annotation.txt"
    #This field can also be set to "NULL"
...
``` r
data = read.delim("example_data/example_discovery_data.txt", nrow = 5)
dim(data)
```
...
If the major cell populations expected in the user-provided discovery dataset are recapitulated by the cell populations analyzed in the EcoTyper carcinoma paper (B cells, CD4 T cells, CD8 T cells, dendritic cells, endothelial cells, epithelial cells, fibroblasts, mast cells, monocytes/macrophages, NK cells, plasma cells and neutrophils), then this field can be set to **Carcinoma\_Fractions** (case sensitive), and EcoTyper will automatically estimate fractions for these populations, in step 1 of the workflow. Similarly, if the cell populations analyzed in the EcoTyper lymphoma paper (B cells , CD4 T cells, CD8 T cells, follicular helper T cells, Tregs, dendritic cells, endothelial cells, fibroblasts, mast cells, monocytes/macrophages, NK cells, plasma cells and neutrophils) are appropriate, then the user can set this field to **Lymphoma\_Fractions** and cell fractions will be automatically calculated.
...
``` r
data = read.delim("example_data/cell_type_fractions_example.txt", nrow = 5)
dim(data)
```
...
``` yaml
#Possible values: "Carcinoma_Fractions", "Lymphoma_Fractions" or a path to a file containing the precomputed cell fractions
Cell type fractions : "example_data/cell_type_fractions_example.txt"
```
...
``` r
annotation = read.delim("example_data/example_discovery_annotation.txt", nrow = 5)
dim(annotation)
```
...
```
    ...
    ##    TCGA.37.A5EN.01A.21R.A26W.07 TCGA.37.4133.01A.01R.1100.07
    ## E1                 7.562699e-01                 6.073491e-02
    ## E2                 1.304320e-02                 6.356203e-02
    ## E3                 8.062408e-02                 7.479170e-03
    ## E4                 3.254682e-15                 6.575619e-03
    ## E5                 2.407759e-02                 1.723206e-14
    ## E6                 1.259853e-01                 8.616483e-01
    ##    TCGA.77.7465.01A.11R.2045.07 TCGA.34.5240.01A.01R.1443.07
    ## E1                 5.452637e-01                 9.796135e-01
    ## E2                 2.724310e-01                 2.683172e-03
    ## E3                 8.950564e-15                 2.081918e-03
    ## E4                 1.079502e-14                 1.562137e-02
    ## E5                 1.591992e-01                 7.382893e-15
    ## E6                 2.310603e-02                 5.405379e-15
    ##    TCGA.05.4249.01A.01R.1107.07
    ## E1                 5.554722e-09
    ## E2                 1.449557e-01
    ## E3                 4.738963e-01
...
```
    ...
    ## 1  TCGA.37.A5EN.01A.21R.A26W.07         E1 0.0050401658 0.024228905
    ## 27 TCGA.63.A5ML.01A.31R.A27Q.07         E1 0.0034770552 0.019316973
    ## 28 TCGA.94.8490.01A.11R.2326.07         E1 0.0553137729 0.115044988
    ## 43 TCGA.66.2800.01A.01R.1201.07         E1 0.1328178840 0.192361392
    ## 65 TCGA.22.4613.01A.01R.1443.07         E1 0.0001134035 0.001090418
    ## 81 TCGA.22.5473.01A.01R.1635.07         E1 0.1278858788 0.189180294
...
**Question**: How do I run EcoTyper on a high-performance cluster, rather than a single server? <br/> **Answer**: EcoTyper can be modified to run on a high-performance cluster, by overriding the `pipeline\lib\multithreading.R` library. Currently the library provides two functions, *PushToJobQueue* which adds a command line call to the job queue, and *RunJobQueue* which waits for all the jobs in the queue to finish. The default implementation of these functions uses R function *mclapply* to perform computations on multiple threads. Users can re-write these two functions according to the requirements of their cluster infrastructure:
